...
static int MakeEccSignature(unsigned char *keybytes, int *keylength, char *name_to_make_key_for, int level, char* prvt_text, char* public_text, bool baboon, HWND log)
{
    EC_PARAMETER Base;
    EC_KEYPAIR Signer;
    SIGNATURE signature;
...
static int MakeSignature(unsigned char *keybytes, int *keylength, char *name_encryptkey, int level, char* pvt_kg_txt, char* y_kg_txt, bool baboon, HWND log)
{
...
static void EncryptSignedKey(unsigned char *keybytes, int keylength, char *encryptkey, HWND log)
{
    char tmp[2048]="";
    char log_msg[1024]="";
