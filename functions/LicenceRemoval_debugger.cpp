 ...
 * 							Global Variables
 **************************************************************************/
static uint32_t    mImageBase      = NULL;
static uint32_t    mEntryPoint     = NULL;
static uint32_t    mSizeOfImage    = NULL;
static HANDLE      mHandle;
static char*       mFileName       = NULL;
...
void LR_GetArmaLicenseData(char parFileName[], vector<ArmaLicenseEntry_t>* parArmaLicenseEntryListPtr)
{
    ...
    LPPROCESS_INFORMATION wProcessInfo = NULL;
    ...
    mImageBase = (uint32_t)GetPE32Data(parFileName, NULL, UE_IMAGEBASE);
    printf("mImageBase : 0x%08X\n", mImageBase);

    mEntryPoint = (uint32_t)GetPE32Data(parFileName, NULL, UE_OEP);
    printf("mEntryPoint : 0x%08X\n", mEntryPoint);

    mSizeOfImage = (uint32_t)GetPE32Data(parFileName, NULL, UE_SIZEOFIMAGE);
    printf("mSizeOfImage : 0x%08X\n", mSizeOfImage);

    // Initialize the debugged process
    wProcessInfo = (LPPROCESS_INFORMATION)InitDebug(parFileName, NULL, NULL);
...
void LR_OpenMutexAArma960Callback()
{
    ...
    else
    {
        ...
        {
            if(strcmp((const char*)(wMutexNameString + wMutexNameLength - 16), (const char*)":SIMULATEEXPIRED") == 0)
            {
...
vector<ArmaLicenseEntry_t> LR_FilterAPIContextContainerListArma960(vector<APIContextContainer_t> parAPIContextContainerList)
{
    ...
    for(wI = 0; wI < wListSize; wI++)
    {
        if(wI < (wListSize-1))
        {
            if((parAPIContextContainerList.at(wI).APIType == REG_OPEN_KEY_EX_A_CONTEXT) && (parAPIContextContainerList.at(wI+1).APIType == REG_QUERY_VALUE_EX_A_CONTEXT))
            {
                if(((HKEY)parAPIContextContainerList.at(wI).RegOpenKeyExAContext.hKey == HKEY_CURRENT_USER) && (parAPIContextContainerList.at(wI).RegOpenKeyExAContext.subKeyStr.compare("Software\\Licenses") == 0))
                {
                    wTempArmaLicenseEntry.Type = REGISTRY_KEY_ENRTY;
 ...
 *              If the key to check is not a value, parKeyValueName need to be a null string.
 *
